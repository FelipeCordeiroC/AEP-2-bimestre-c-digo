...
int gerar_token() {
    srand(time(NULL));
    return rand() % 9000 + 1000;  // Gera um número aleatório de 4 dígitos entre 1000 e 9999
}

void registrar() {
    char usuario[100], senha[100], telefone[20], chave = 0xAA;
...
void login() {
    ...
    while (fscanf(arquivo, "%s %s %s", nomeUsuario, senhaCriptografada, telefoneRegistrado) != EOF) {
        ...
        if (strcmp(usuario, nomeUsuario) == 0 && strcmp(senha, senhaCriptografada) == 0) {
            ...
            // Geração e simulação de envio do token
...
void alterar_usuario() {
    ...
    char novoUsuario[100], novaSenha[100], novoTelefone[20];
    char usuario[100], senha[100], telefone[20];
    char chave = 0xAA;
    int usuarioEncontrado = 0;
    ...
    while (fscanf(arquivo, "%s %s %s", usuario, senha, telefone) != EOF) {
        if (strcmp(usuario, usuario_alterar) == 0) {
            ...
            fprintf(temp, "%s %s %s\n", novoUsuario, novaSenha, novoTelefone);
            usuarioEncontrado = 1;
        } else {
            fprintf(temp, "%s %s %s\n", usuario, senha, telefone);
        }
